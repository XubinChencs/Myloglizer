...
public class PredictResult<T> {
     ...
     * 总数
     */
	private static int positive_cnt = 0;
     ...
     */
    public List<String> display(long items) {
        if (!valid) {
        ...
        for (int i = 0; i < items; i++) {
            ...
            percent /= sumCount;
            res.add("Predict:" + keys.get(i).key + " " + "Prob("+ keys.get(i).count + "/"+ sumCount + "):"+ (double) Math.round(percent * 100) / 100 + "%");
    ...
    public List<String> display() {
        return display(keys.size());
    }

    
    public boolean judge(T k) {
    	for (int i = 0; i < keys.size(); i++) {
    		if (keys.get(i).key.equals(k)) {
    			double percent = keys.get(i).count * 100;
                percent /= sumCount;
                if (percent > 20) {
                	return true;
    ...
    public boolean judgeByMax(T k) {
    	if (keys.size() > 0) {
    		if (keys.get(0).key.equals(k))
    			return true;
    	}
    	return false;
    }
    
    public boolean judgeByDiff(T k) {
    	total_cnt++;
    	System.out.println("t: " + total_cnt);
		System.out.println("p: " + positive_cnt);
    	if (keys.size() > 0) {
    		positive_cnt++;
    		if (Math.abs((Double)keys.get(0).key - (Double)k ) < 0.1)
    			return true;
    	}
    	return false;
    }
