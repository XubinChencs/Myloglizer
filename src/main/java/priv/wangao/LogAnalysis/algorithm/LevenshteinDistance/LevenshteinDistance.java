...
public class LevenshteinDistance {
	 ...
	 * @return
	 */
    public static int[][] getMinDistanceMatrix(String[] word1, String[] word2) {
    ...
    public static EditPath getOnePath(String[] word1, String[] word2, int[][] distance) {
		...
		while(i > 0 && j > 0) {
			String char1 = word1[i - 1];
			String char2 = word2[j - 1];
    ...
    public static AlignStringPair align(String[] word1, String[] word2, EditPath editPath) {
    	...
    	for(int i = 1; i <= size - 1; i++) {
    		...
    		if (iMinus == 0 && jMinus == 1) {
				newWord1.append("_");
				newWord2.append(word2[now.getJ() - 1]);
			// 删除
			} else if (iMinus == 1 && jMinus == 0) {
				newWord1.append(word1[now.getI() - 1]);
				newWord2.append("_");
			// 相等或者修改
			} else {
				newWord1.append(word1[now.getI() - 1]);
    	...
    	AlignStringPair alignStringPair = new AlignStringPair(
    			newWord1.toString().trim().split(" "), newWord2.toString().trim().split(" "));
...
//		Printer.print2DArray(test);
		...
		String str1 = "abc qba qwer asd asd asd 1234 4321 ada dasa";
		String str2 = "abc qwer asd 4321";
